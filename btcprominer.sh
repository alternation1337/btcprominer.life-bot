...
signup=$(curl -s -X POST -d "address=$address&pin=$pin&action=post&refid=$Reff" --user-agent "Mozilla/5.0 (X11; Ubuntu; Linux i686; rv:53.0) Gecko/20100101 Firefox/53.0" "https://btcprominer.life/" -e "https://web.facebook.com/" -x "$proxys" -m 10 2> /dev/null)
    if [[ $signup == "" ]]
        then
